...
export default function DoctorAccess() {
  ...
  async function handleAccess(e: React.FormEvent) {
    ...
    // 1. Find patient
    const { data: patient, error: pErr } = await dataTable('patients')
      .select('*')
    ...
    const { data: records } = await dataTable('medical_records')
      .select('*')
      .eq('hid_code', hidCode.trim().toUpperCase())
      .order('created_at', { ascending: false })

    // 4. Log access
    await dataTable('access_logs').insert({
  ...
  async function handleAddRecord(e: React.FormEvent) {
    ...
    const { data: newRecord, error } = await dataTable('medical_records')
      .insert({
          ...
          <div style={{ display: 'flex', flexDirection: 'column', gap: 16, animation: 'fadeIn 0.3s ease' }}>
            ...
            <Card>
              ...
              <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4, 1fr)', gap: 16, background: '#f9fafb', borderRadius: 10, padding: 16 }}>
                {[
                  { label: 'Date of Birth', value: data.patient.dob ? formatDate(data.patient.dob) : 'Not set' },
