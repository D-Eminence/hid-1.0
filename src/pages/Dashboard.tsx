                  ...
                  {stats.recentPatients.map(p => (
                    <div key={p.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 600, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{p.full_name ?? 'N/A'}</div>
                        <div style={{ fontSize: 11, color: '#9ca3af', fontFamily: 'monospace' }}>{p.hid_code ?? 'N/A'}</div>
                      </div>
                      <Badge color="blue">{p.blood_group ?? 'N/A'}</Badge>
                    </div>
                  ))}
                </div>
              )}
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: 12 }}>
                  {stats.recentLogs.map(log => (
                    <div key={log.id} style={{ display: 'flex', alignItems: 'center', gap: 12 }}>
                      ...
                      <div style={{ flex: 1, minWidth: 0 }}>
                        <div style={{ fontWeight: 500, fontSize: 13, overflow: 'hidden', textOverflow: 'ellipsis', whiteSpace: 'nowrap' }}>{log.accessed_by ?? 'N/A'}</div>
                        <div style={{ fontSize: 11, color: '#9ca3af', fontFamily: 'monospace' }}>{log.hid_code ?? 'N/A'}</div>
                      </div>
