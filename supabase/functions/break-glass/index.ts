...
Deno.serve(req => withErrorHandling(req, async () => {
  ...
  if (!staffAccount?.role) {
    throw new HttpError(403, 'Only hospital staff can trigger emergency access.')
  }
  await assertPlatformFeatureEnabled(adminClient, 'break_glass')
  await assertStaffRoleCapability(adminClient, staffAccount.role, 'can_use_break_glass')
