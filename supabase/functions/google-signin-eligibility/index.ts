import { createAdminClient } from '../_shared/auth.ts'
import { HttpError, json, readJson, withErrorHandling } from '../_shared/http.ts'
import { optionalTrimmedString } from '../_shared/validation.ts'

type Payload = {
  accountType?: 'patient' | 'hospital'
  email?: string | null
}

Deno.serve(req => withErrorHandling(req, async () => {
  if (req.method !== 'POST') throw new HttpError(405, 'Method not allowed.')

  const body = await readJson<Payload>(req)
  const accountType = body.accountType === 'hospital' ? 'hospital' : 'patient'
  const email = optionalTrimmedString(body.email)?.toLowerCase() ?? ''
  if (!/\S+@\S+\.\S+/.test(email)) {
    throw new HttpError(400, 'Enter the email address already registered on HID before using Google sign-in.')
  }

  const adminClient = createAdminClient()
  const result = accountType === 'hospital'
    ? await adminClient.from('hid_staff_accounts').select('id').eq('email', email).is('deleted_at', null).limit(1)
    : await adminClient.from('hid_patients').select('id').eq('email', email).is('deleted_at', null).limit(1)

  if (result.error) throw new HttpError(400, 'We could not verify this HID account right now.', result.error)

  return json({ data: { accountType, email, registered: (result.data ?? []).length > 0 } })
}))
