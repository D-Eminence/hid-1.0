...
alter table public.hid_organizations
  add column if not exists state text;

alter table public.hid_organizations
  add column if not exists country text;

create unique index if not exists idx_hid_organizations_name_region
  on public.hid_organizations (lower(name), lower(state), lower(country))
...
begin
  ...
  if requested_role in ('clinician', 'platform_admin') then
    if not exists (
      select 1
...
begin
  ...
  requested_role := coalesce(auth_metadata ->> 'requested_role', 'patient');
  pending_onboarding := coalesce(auth_metadata -> 'pending_staff_onboarding', '{}'::jsonb);
  onboarding_type := coalesce(nullif(trim(pending_onboarding ->> 'onboardingType'), ''), 'staff_invite');
  ...
  if invite_row.id is not null then
    ...
    hospital_name_value := nullif(trim(pending_onboarding ->> 'hospitalName'), '');
    state_value := nullif(trim(pending_onboarding ->> 'state'), '');
    country_value := nullif(trim(pending_onboarding ->> 'country'), '');
    ...
    values (organization_id, hospital_name_value, upper(substring(md5(organization_id::text) from 1 for 10)))
...
grant execute on function public.hid_complete_staff_onboarding(text, text) to authenticated;
